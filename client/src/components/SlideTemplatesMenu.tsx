...
interface SlideTemplatesMenuProps {
  onApplyTemplate: (templateId: string) => void;
  onAddSlide: (templateId: string) => void;
}

export default function SlideTemplatesMenu({ onApplyTemplate, onAddSlide }: SlideTemplatesMenuProps) {
  const [selectedAction, setSelectedAction] = useState<'apply' | 'add'>('add');
    ...
    <div className="w-full h-full flex flex-col">
      ...
      <div className="p-4 border-b">
        ...
        <p className="text-sm text-gray-600">
          {selectedAction === 'add' 
            ? 'Choose a template to add a new slide'
            : 'Choose a template to apply to current slide'
          }
        </p>
      </div>
